...
$query="insert into  registration(roomno,sharing,feespm,foodstatus,stayfrom,duration,firstName,middleName,lastName,gender,contactno,emailid,egycontactno,corresAddress,corresCIty,corresState,corresPincode,pmntAddress,pmntCity,pmnatetState,pmntPincode) values(?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)";
$stmt = $mysqli->prepare($query);
$rc=$stmt->bind_param('iiiisissssisisssisssi',$roomno,$sharing,$feespm,$foodstatus,$stayfrom,$duration,$fname,$mname,$lname,$gender,$contactno,$emailid,$emcntno,$caddress,$ccity,$cstate,$cpincode,$paddress,$pcity,$pstate,$ppincode);
$stmt->execute();
...
<head>
	...
	<title>User PG Registration</title>
...
function calculateTotalAmount() {
    var fpm = parseInt(document.getElementById("fpm").value) || 0;
    var duration = parseInt(document.getElementById("duration").value) || 0;
    var foodStatus = document.querySelector('input[name="foodstatus"]:checked')?.value || 0;
    
    var foodCharge = (foodStatus == 1) ? 2000 : 0;
    var total = duration * (fpm + foodCharge);

    document.getElementById("ta").value = total;
}

// Trigger calculation on each relevant change
document.addEventListener("DOMContentLoaded", function() {
    ...
    let foodRadios = document.getElementsByName("foodstatus");
    foodRadios.forEach(function(radio) {
						...
						<div class="row">
							...
							<h3 style="color: red" align="left">Room already booked by you</h3>
								<?php }
										...
										<div class="col-sm-8">
										<select name="room" id="room"class="form-control"  onChange="getSharing(this.value);" onBlur="checkAvailability()" required> 
										...
										<div class="col-sm-8">
										<input type="text" name="sharing" id="sharing"  class="form-control"  >
										</div>
										</div>
										...
										<div class="form-group">
											...
											<div class="col-sm-8"> 
												<?php 
													$totalAmount = 0; // Initialize to avoid "undefined variable" warning

													if(isset($_POST['duration']) && isset($_POST['fpm'])) {
														$duration = $_POST['duration'];
														$feespm = $_POST['fpm'];
														$foodstatus = $_POST['foodstatus'];

														if($foodstatus == 1) { // With food
															$totalAmount = ($duration * $feespm) + (2000 * $duration);
														} else { // Without food
															$totalAmount = $duration * $feespm;
														}
													}
												?>

												<input type="text" name="ta" id="ta" value="<?php echo $totalAmount; ?>" class="result form-control" disabled>
											</div>
										</div>
